...
import { addDays, dateFromText, defaultSettings, getNextLoanStatus, previewRepayment, type CalculatorSettingsInput } from "@/lib/debt";
...
export async function POST(request: Request, { params }: { params: { slug: string; loanId: string } }) {
  try {
    ...
    if (loan.status === "closed") return NextResponse.json({ error: "案件已結清" }, { status: 400 });
    ...
    const paymentDate = dateFromText(String(body.paymentDate));
    const baseDate = loan.lastInterestCalcDate ?? loan.startDate;
